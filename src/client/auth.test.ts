...
describe("OAuth Authorization", () => {
  ...
  describe("discoverOAuthProtectedResourceMetadata", () => {
    ...
    it("throws an error when all fetch attempts fail", async () => {
      ...
      await expect(discoverOAuthProtectedResourceMetadata("https://resource.example.com", userAgentProvider))
    ...
    it("preserves query parameters in path-aware discovery", async () => {
      ...
      const metadata = await discoverOAuthProtectedResourceMetadata("https://resource.example.com/path?param=value", userAgentProvider);
      expect(metadata).toEqual(validMetadata);
      const calls = mockFetch.mock.calls;
      expect(calls.length).toBe(1);
      const [url] = calls[0];
    ...
    it("does not fallback when resourceMetadataUrl is provided", async () => {
      ...
      await expect(discoverOAuthProtectedResourceMetadata("https://resource.example.com/path", userAgentProvider, {
  ...
  describe("discoverOAuthMetadata", () => {
    ...
    it("throws an error when all fetch attempts fail", async () => {
      ...
      // Should fail with the second error
      await expect(discoverOAuthMetadata("https://auth.example.com", userAgentProvider))
        .rejects.toThrow("Second failure");
    ...
    it("throws on non-404 errors", async () => {
      ...
      await expect(
        discoverOAuthMetadata("https://auth.example.com", userAgentProvider)
      ).rejects.toThrow("HTTP 500");
    });
  ...
  describe("discoverAuthorizationServerMetadata", () => {
    ...
    it("throws on non-4xx errors", async () => {
      ...
      await expect(
        discoverAuthorizationServerMetadata("https://mcp.example.com", userAgentProvider)
      ).rejects.toThrow("HTTP 500");
    });
