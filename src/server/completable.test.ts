import { z } from 'zod';
import { completable, completableRegistry } from './completable.js';

const fallback = {
    complete: () => {
        throw new Error('No completion function found');
    }
};
describe('completable', () => {
    ...
    it('provides access to completion function', async () => {
        ...
        const { complete } = completableRegistry.get(schema) ?? fallback;
        expect(await complete('')).toEqual(completions);
    });
    ...
    it('passes current value to completion function', async () => {
        ...
        const { complete } = completableRegistry.get(schema) ?? fallback;
        expect(await complete('test')).toEqual(['test!']);
    });
    ...
    it('works with number schemas', async () => {
        ...
        const { complete } = completableRegistry.get(schema) ?? fallback;
        expect(await complete(0)).toEqual([1, 2, 3]);
    });
