...
| Streamable HTTP server (stateless)        | Shared instance for 2026-06 clients. Pre-2026 clients need per-instance isolation: per-session (see stateful row) or a fresh server per request. | [`src/simpleStatelessStreamableHttp.ts`](src/simpleStatelessStreamableHttp.ts)           |
