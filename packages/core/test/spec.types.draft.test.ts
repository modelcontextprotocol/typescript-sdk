...
const sdkTypeChecks = {
    ...
    // The SDK models the draft's required per-request `_meta` envelope as
...
const DRAFT_MISSING_SDK_TYPES = [
    ...
    // SEP-2575 per-request envelope: draft requests REQUIRE a `_meta` envelope
    ...
    // SEP-2322 (MRTR) → PR for MRTR: draft results carry a required `resultType`
    ...
    // (DiscoverRequestSchema / DiscoverResultSchema), but the draft shapes embed the
    ...
    // Draft subscriptions surface (new) → PR for subscriptions/listen:
    'SubscriptionFilter',
    ...
    // Other shapes changed by the draft schema: sampling content changes (SamplingMessage,
...
describe('Spec Types (draft)', () => {
    ...
    it('pins the per-request _meta envelope keys to the draft schema', () => {
