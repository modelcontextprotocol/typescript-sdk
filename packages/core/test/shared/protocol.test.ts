...
import type { ClientCapabilities, JSONRPCMessage, Request, ServerCapabilities } from '../../src/types/index.js';
...
function createTestProtocol(options?: ConstructorParameters<typeof TestProtocolImpl>[0]): TestProtocolImpl {
    return new TestProtocolImpl(options);
}
...
describe('protocol tests', () => {
    ...
    describe('Debounced Notifications', () => {
        ...
        it('should debounce calls when params is explicitly undefined', async () => {
            ...
            expect(sendSpy.mock.calls[0]![0]).not.toHaveProperty('params');
        });
