...
// ───────────────────────────────────────────────────────────────────────────────
// DELETED 2025-11 task-mechanism test suites (R0; not skipped, mechanism removed)
// ───────────────────────────────────────────────────────────────────────────────
// These suites tested Protocol's task *interception machinery* (processInbound*/
// processOutbound*, _taskManager, relatedTask queueing). R0 removes that machinery
// because SEP-2663 (https://github.com/modelcontextprotocol/modelcontextprotocol/issues/2663)
// changes the task model in ways that make every one of these mechanisms unreachable:
//
//   Removed describe                          Why no SEP-2663 equivalent
//   ────────────────────────────────────────  ────────────────────────────────────
//   InMemoryTaskMessageQueue                  No outbound queueing; server returns
//                                             {resultType:'task'} directly.
//   Task-based execution / request with task  Client never sends params.task
//                                             (server-directed model).
//   relatedTask metadata / combination        No relatedTask side-channel; task id
//                                             is in result body, not _meta.
//   assertTaskHandlerCapability               Capability check is on tasks/* method
//                                             registration, not per-request assert.
//   pollInterval / _waitForTaskUpdate         Polling moves to client-side helper
//                                             (tasksPlugin); not Protocol concern.
//   listTasks / cancelTask / status notifs    tasks/* are plugin handlers, not
//                                             Protocol-level interception.
//   Request vs Task Cancellation separation   notifications/cancelled is the only
//                                             cancel; tasks/cancel is a request.
//   Progress notification support for tasks   Progress via standard
//                                             notifications/progress; no task tap.
//   Capability negotiation for tasks          ServerCapabilities.tasks; no inbound
//                                             rewriting on negotiate.
//   Message interception (×3 suites)          processInbound*/processOutbound*
//                                             gone; plugin uses use() middleware.
//
// Tests for the SEP-2663 server-directed model land with that implementation; nothing in
// this commit re-covers task behaviour.
// ───────────────────────────────────────────────────────────────────────────────
