...
const sdkTypeChecks = {
    ...
    ClientNotification: (sdk: WithJSONRPC<SDKTypes.ClientNotification>, spec: SpecTypes.ClientNotification) => {
        // @ts-expect-error SDK removed TaskStatusNotification (tasks removed from SDK ahead of spec)
        sdk = spec;
        spec = sdk;
    },
    ...
    ClientRequest: (sdk: WithJSONRPCRequest<SDKTypes.ClientRequest>, spec: SpecTypes.ClientRequest) => {
        // @ts-expect-error SDK removed task request types (tasks removed from SDK ahead of spec)
        sdk = spec;
        spec = sdk;
    },
...
// @ts-expect-error SDK removed 'task' key (tasks removed from SDK ahead of spec)
...
// @ts-expect-error SDK removed 'tasks' key (tasks removed from SDK ahead of spec)
...
// Task types that exist in the generated spec types but were removed from the
// SDK ahead of the spec (the SDK no longer ships the TaskManager system).
// They are excluded from both the mutual-assignability and key-parity guards.
const TASK_TYPES_REMOVED_FROM_SDK = [
...
describe('Spec Types', () => {
    ...
    const typesToCheck = specTypes.filter(type => !MISSING_SDK_TYPES.includes(type) && !TASK_TYPES_REMOVED_FROM_SDK.includes(type));
    ...
    it('should have up to date list of task types removed from sdk', () => {
        for (const typeName of TASK_TYPES_REMOVED_FROM_SDK) {
            expect(sdkTypes).not.toContain(typeName);
        }
    });
