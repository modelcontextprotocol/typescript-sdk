...
/* Runtime notification schema lookup */
...
function buildNotificationSchemaMap<T extends { shape: { method: { value: string } } }>(schemas: readonly T[]): Record<string, T> {
...
const notificationSchemas = buildNotificationSchemaMap([...ClientNotificationSchema.options, ...ServerNotificationSchema.options] as const) as Record<
    NotificationMethod,
