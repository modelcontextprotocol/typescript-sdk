/**
 * HTTP Server Transport (Web Standards)
 *
 ...
 *
 * For Node.js HTTP compatibility, use `StreamableHTTPServerTransport` from `@modelcontextprotocol/node`.
 * For Express-specific features, use `@modelcontextprotocol/express`.
 */
...
export interface HTTPServerTransportOptions {
     ...
     * HTTPServerTransport after a request is completed while still keeping the
...
/**
 * Server transport for Streamable HTTP using Web Standard APIs (Request, Response, ReadableStream).
 ...
 * const statefulTransport = new HTTPServerTransport({
 *   sessionIdGenerator: () => crypto.randomUUID(),
 * });
 *
 * // Stateless mode - explicitly set session ID to undefined
 * const statelessTransport = new HTTPServerTransport({
...
/**
 * @deprecated Use `HTTPServerTransport` instead. This alias is provided for backwards compatibility.
 */
export const WebStandardStreamableHTTPServerTransport = HTTPServerTransport;

/**
 * @deprecated Use `HTTPServerTransportOptions` instead. This alias is provided for backwards compatibility.
 */
export type WebStandardStreamableHTTPServerTransportOptions = HTTPServerTransportOptions;
