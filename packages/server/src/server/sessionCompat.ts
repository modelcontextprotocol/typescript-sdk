 ...
 * protocol. One instance is shared across all requests to a given {@linkcode shttpHandler}.
 *
...
export class SessionCompat {
    ...
    async validate(req: Request, messages: JSONRPCMessage[]): Promise<SessionValidation> {
        ...
        if (isInit) {
            ...
            const protocolVersion = initMsg && isInitializeRequest(initMsg) ? initMsg.params.protocolVersion : undefined;
    ...
    /** Registers the open standalone GET stream controller for this session. */
    setStandaloneStream(sessionId: string, controller: ReadableStreamDefaultController<Uint8Array> | undefined): void {
        const entry = this._sessions.get(sessionId);
        if (entry) entry.sseController = controller;
    }
