...
export class SessionCompat {
    ...
    /**
     * Registers an additional listener fired when a session is deleted or evicted. Callers
     * may wire this to per-session resources (e.g. `BackchannelCompat.closeSession`) so
     * pending state is rejected on idle/capacity eviction, not only on explicit DELETE.
     */
    ...
    addStreamId(sessionId: string, streamId: string, opts?: { protected?: boolean }): void {
        ...
        while (entry.streamIds.size > MAX_STREAM_IDS_PER_SESSION) {
            let evicted: string | undefined;
            for (const id of entry.streamIds) {
                if (!entry.protectedStreamIds.has(id)) {
                    evicted = id;
                    break;
                }
            }
            if (evicted === undefined) break;
            entry.streamIds.delete(evicted);
        }
    }

    /**
     * Forgets a stream ID. Exposed for store/subclass implementations that want explicit
     * cleanup; `shttpHandler` itself does not call this (stream IDs are kept for
     * `Last-Event-ID` resumability and bounded by {@linkcode addStreamId}'s LRU cap).
     */
