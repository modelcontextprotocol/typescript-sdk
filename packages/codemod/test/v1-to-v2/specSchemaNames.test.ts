...
describe('SPEC_SCHEMA_NAMES (codemod schema-routing allowlist)', () => {
    it("matches @modelcontextprotocol/sdk-shared's exported schema set exactly (drift guard)", () => {
        // The import transform routes a `*Schema` symbol from sdk/types.js to sdk-shared only when the
        // symbol's (rename-resolved) name is in this set. It must therefore equal sdk-shared's actual
        // public exports: a name missing here would be misrouted to client/server (which export no Zod
        // schema values), and a name here that sdk-shared does not export would produce a broken import.
        // Read sdk-shared's barrel directly so the two cannot silently drift.
