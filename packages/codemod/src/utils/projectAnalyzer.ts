...
import { info, warning } from './diagnostics.js';
...
export function analyzeProject(targetDir: string): TransformContext {
    ...
    if (pkgJsonPath) {
        try {
            ...
            // No v2 split deps yet — this is almost always a v1 project mid-migration (the v1 SDK is
            // a single `@modelcontextprotocol/sdk` package). Fall through to inferring the type from
            // how the source actually uses the SDK.
        } catch {
 ...
 * `@modelcontextprotocol/sdk/client/...` import means the project ends up needing
 * `@modelcontextprotocol/client`; a `.../server/...` import means it needs `@modelcontextprotocol/server`.
 * Files that import only shared paths (`types.js`, `shared/...`) give no signal. Bounded and early-exits
 * once both signals are seen.
 */
    ...
    const visit = (dir: string): void => {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (content.includes('@modelcontextprotocol/sdk/client/')) usesClient = true;
                if (content.includes('@modelcontextprotocol/sdk/server/')) usesServer = true;
            }
        }
    };
...
export function resolveTypesPackage(
    ...
    if (context.projectType === 'both') {
        // Both packages are present and both re-export the shared protocol types from core, so
        // importing them from either compiles. This file has no client/server-specific signal, so
        // default to server and note it — no manual action is required, only an optional preference.
        if (diagnosticSink) {
