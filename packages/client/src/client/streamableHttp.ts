...
export type StreamableHTTPClientTransportOptions = {
    ...
    /**
     * Customizes HTTP requests to the MCP server (the data-plane `POST`/`GET`/`DELETE`
     ...
     * {@linkcode StreamableHTTPClientTransportOptions.oauthRequestInit | oauthRequestInit}
     ...
     * {@linkcode StreamableHTTPClientTransportOptions.requestInit | requestInit} are
     * never applied to these requests; set this option when they need extra
     * configuration (e.g. gateway headers on well-known endpoints).
     *
...
export class StreamableHTTPClientTransport implements Transport {
    ...
    async finishAuth(codeOrParams: string | URLSearchParams, iss?: string): Promise<void> {
        ...
        const result = await auth(this._oauthProvider, {
            ...
            resourceMetadataUrl: this._resourceMetadataUrl,
            // `_resourceMetadataUrl` is only ever populated from a WWW-Authenticate
            // challenge — see `_stepUpAuthorize`.
            resourceMetadataUrlSource: 'www-authenticate',
            scope: this._scope,
            fetchFn: this._authFetch,
            skipIssuerMetadataValidation: this._skipIssuerMetadataValidation
