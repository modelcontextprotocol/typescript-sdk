...
export class StreamableHTTPClientTransport implements Transport {
    ...
    private _applyBodyDerivedHeaders(headers: Headers, message: JSONRPCMessage | JSONRPCMessage[]): void {
        ...
        // SEP-2792: mirror _meta[acceptLanguage] → Accept-Language header.
        const acceptLanguage = meta?.[ACCEPT_LANGUAGE_META];
    ...
    /**
     * SEP-2792: Extract `contentLanguage` from a JSON-RPC response message's
     * `_meta` (for result responses) or `error.data._meta` (for error responses).
     */
    private _extractContentLanguageMeta(msg: JSONRPCMessage): string | undefined {
        if (isJSONRPCResultResponse(msg)) {
            const meta = (msg.result as { _meta?: Record<string, unknown> } | undefined)?._meta;
            const v = meta?.[CONTENT_LANGUAGE_META];
            return typeof v === 'string' ? v : undefined;
        }
        if (isJSONRPCErrorResponse(msg)) {
            const data = msg.error.data as { _meta?: Record<string, unknown> } | undefined;
            const v = data?._meta?.[CONTENT_LANGUAGE_META];
            return typeof v === 'string' ? v : undefined;
        }
        return undefined;
    }
        ...
        try {
            ...
            if (hasRequests) {
                if (responseMediaType === 'text/event-stream') {
                    ...
                    // SEP-2792: verify Content-Language ↔ _meta byte-equality on JSON responses.
                    const contentLanguageHeader = response.headers.get('content-language') ?? undefined;
                    if (contentLanguageHeader !== undefined) {
                        for (const msg of responseMessages) {
                            const metaContentLang = this._extractContentLanguageMeta(msg);
                            if (metaContentLang !== undefined && contentLanguageHeader !== metaContentLang) {
                                throw new SdkError(
                                    SdkErrorCode.ClientHttpUnexpectedContent,
                                    `Content-Language header "${contentLanguageHeader}" does not match ` +
                                        `_meta['io.modelcontextprotocol/contentLanguage'] "${metaContentLang}"`,
                                    { header: contentLanguageHeader, meta: metaContentLang }
                                );
                            }
                        }
                    }
