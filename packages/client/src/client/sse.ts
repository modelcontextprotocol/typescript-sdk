...
import { createTransportHttp, crossOriginSseGetHeaders } from './transportRedirect';
                ...
                fetch: async (url, init) => {
                    ...
                    try {
                        response = await this._http.eventSourceGet({
                            ...
                            crossOriginHeaders: crossOriginSseGetHeaders(this._protocolVersion)
                        });
                    } catch (error) {
                        if (error instanceof SdkHttpError && error.code === SdkErrorCode.ClientHttpRedirectNotFollowed) {
                            ...
                            try {
                                ...
                                // A throwing onerror must not skip the close.
                                void this.close();
                            }
                        }
                        throw error;
