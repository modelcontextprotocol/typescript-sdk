 ...
 *
 * For web-standard environments (Cloudflare Workers, Deno, Bun), use `HTTPServerTransport` directly from `@modelcontextprotocol/server`.
 */
...
import type { HTTPServerTransportOptions } from '@modelcontextprotocol/server';
import { HTTPServerTransport } from '@modelcontextprotocol/server';

/**
 * Configuration options for StreamableHTTPServerTransport
 *
 * This is an alias for HTTPServerTransportOptions for backward compatibility.
 */
export type StreamableHTTPServerTransportOptions = HTTPServerTransportOptions;

/**
 * Server transport for Streamable HTTP: this implements the MCP Streamable HTTP transport specification.
 * It supports both SSE streaming and direct HTTP responses.
 *
 * This is a wrapper around `HTTPServerTransport` that provides Node.js HTTP compatibility.
...
export class StreamableHTTPServerTransport implements Transport {
    private _httpServerTransport: HTTPServerTransport;
    ...
    constructor(options: StreamableHTTPServerTransportOptions = {}) {
        this._httpServerTransport = new HTTPServerTransport(options);
        ...
        this._requestListener = getRequestListener(async (webRequest: Request) => {
            ...
            return this._httpServerTransport.handleRequest(webRequest, {
    ...
    get sessionId(): string | undefined {
        return this._httpServerTransport.sessionId;
    }
    ...
    set onclose(handler: (() => void) | undefined) {
        this._httpServerTransport.onclose = handler;
    }

    get onclose(): (() => void) | undefined {
        return this._httpServerTransport.onclose;
    }
    ...
    get onerror(): ((error: Error) => void) | undefined {
        return this._httpServerTransport.onerror;
    }
    ...
    set onmessage(handler: ((message: JSONRPCMessage, extra?: MessageExtraInfo) => void) | undefined) {
        this._httpServerTransport.onmessage = handler;
    }

    get onmessage(): ((message: JSONRPCMessage, extra?: MessageExtraInfo) => void) | undefined {
        return this._httpServerTransport.onmessage;
    }
    ...
    async start(): Promise<void> {
        return this._httpServerTransport.start();
    }
    ...
    async close(): Promise<void> {
        return this._httpServerTransport.close();
    }
    ...
    async send(message: JSONRPCMessage, options?: { relatedRequestId?: RequestId }): Promise<void> {
        return this._httpServerTransport.send(message, options);
    }
     ...
     * and delegates to the underlying HTTPServerTransport.
     *
    ...
    closeSSEStream(requestId: RequestId): void {
        this._httpServerTransport.closeSSEStream(requestId);
    }
    ...
    closeStandaloneSSEStream(): void {
        this._httpServerTransport.closeStandaloneSSEStream();
    }
}
