...
`SdkError(InputRequiredRoundsExceeded)` when the cap is exhausted. The manually
returned `InputRequiredResult` retains base result fields, including `_meta` and
extension fields.
