...
structural read with an unvalidated cast), three typed readers ship from
...
- `samplingText(responses, key)` — the text of a sampling response (first text block
  for with-tools array content), or `undefined`.
...
| --- | --- | --- |
| `maxRounds` | `8` | Handler re-entries per originating request before failing |
...
When the originating request carried a `progressToken`, the shim emits one synthetic
`notifications/progress` per completed round against it, so a deployed 2025 client
composing `resetTimeoutOnProgress` around its call sees liveness across a long
multi-round flow.
...
- JSON-mode legacy clients (`enableJsonResponse`) cannot receive server→client
  requests mid-call, exactly as with today's `elicitInput`.
