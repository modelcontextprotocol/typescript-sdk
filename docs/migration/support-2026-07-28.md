...
(Typings published before `2.0.0-beta.1` — the alphas — predate this exclusion:
there the typed maps still carry the `tasks/*` entries and the `CreateTaskResult`
unions; narrow with the `isCallToolResult` guard if you are pinned to an alpha.) Where
