...
// `params.task` and no interception. The behaviors that survive are covered by
// `taskLifecycle.test.ts` (tasksPlugin) and `experimental/tasks/taskListing.test.ts`.
//
...
// SEP-2663 removes the client-hosted task store these tests relied on (server polls
// the client's tasks/* for async elicitation). That direction becomes MRTR (S3/F4),
// not tasks. The `elicitInputStream` wrapper has no equivalent and is removed.
