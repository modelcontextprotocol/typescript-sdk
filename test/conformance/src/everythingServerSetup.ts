 ...
 * harness exercises the same handler set against:
 *  - {@linkcode ./everythingServer.ts} — `transport.connect()` / `NodeStreamableHTTPServerTransport`
 *  - {@linkcode ./everythingServerHandleStatelessHttp.ts} — `handleStatelessHttp()` (2026-06 only)
 */
 ...
 * per session (transport.connect path) or per request (handleStatelessHttp path).
 */
