...
const CORE_TYPES_DIR = join(PROJECT_ROOT, 'packages', 'core', 'src', 'types');
const SPEC_TYPES_FILE = join(CORE_TYPES_DIR, 'spec.types.ts');
const GENERATED_DIR = join(CORE_TYPES_DIR, 'generated');
const SDK_TYPES_FILE = join(GENERATED_DIR, 'sdk.types.ts');
...
// DO NOT EDIT - Run: pnpm generate:schemas`
    );
...
function topLevelDescribe(sourceFile: SourceFile): void {
    ...
    for (const varStmt of sourceFile.getVariableStatements()) {
        ...
        // Normalize newlines to spaces, then use JSON.stringify for safe string literal embedding.
        // JSON.stringify handles all escaping (backslashes, quotes, control chars) in one step.
        const normalizedDesc = descText.replace(/\n/g, ' ');

        // Add .describe() to the schema
        decl.setInitializer(`${currentText}.describe(${JSON.stringify(normalizedDesc)})`);
        count++;
    }
...
${cleanedTypesContent.replace(/^\/\*\*[\s\S]*?\*\/\n/, '')}`;
    ...
    // Format generated files with prettier. The generated dir is in
    // .prettierignore (so `lint` doesn't check it), but the generator itself
    // still wants deterministic formatting for readable diffs — bypass the
    // ignore file here.
    console.log('\n📝 Formatting generated files...');
    execSync(`pnpm exec prettier --ignore-path /dev/null --write "${GENERATED_DIR}/**/*"`, {
        cwd: PROJECT_ROOT,
        stdio: 'inherit'
    });
