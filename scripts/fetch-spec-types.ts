 ...
 * - `draft`: the in-progress schema for the next protocol revision.
 *
 * Each maps to `schema/<version>/schema.ts` upstream and is written to
 * `packages/core/src/types/spec.types.<version>.ts`.
 */
const SUPPORTED_VERSIONS = ['2025-11-25', 'draft'] as const;
...
async function fetchLatestSHA(version: SpecVersion): Promise<string> {
    const url = `https://api.github.com/repos/modelcontextprotocol/modelcontextprotocol/commits?path=schema/${version}/schema.ts&per_page=1`;
...
async function fetchSpecTypes(version: SpecVersion, sha: string): Promise<string> {
    const url = `https://raw.githubusercontent.com/modelcontextprotocol/modelcontextprotocol/${sha}/schema/${version}/schema.ts`;
 ...
 * Pulled from: https://raw.githubusercontent.com/modelcontextprotocol/modelcontextprotocol/main/schema/{VERSION}/schema.ts
 ...
 */`;
    ...
    const header = headerTemplate.replaceAll('{VERSION}', version).replace('{SHA}', sha);
