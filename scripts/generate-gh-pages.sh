...
generate_docs() {
    ...
    (cd "${source_dir}" && npx typedoc --out "${output_dir}")
...
# Determine if this tag is the latest version and update static files if so
LATEST_VERSION="v$(head -1 _data/versions.yml | sed 's/^- //')"
